                    ...
                    <div class="mt-4 flex space-x-2 items-center">
                        @if($book->stock > 0)
                            <form action="{{ route('member.loans.store') }}" method="POST">
                                ...
                                <button class="px-4 py-2 bg-blue-600 text-white rounded">Pinjam Buku</button>
                            </form>
                        @else
                            <span class="text-red-500 text-sm">Buku sedang tidak tersedia</span>
                        @endif
                        <a href="{{ route('member.books.index') }}" class="px-4 py-2 bg-gray-200 rounded">Kembali</a>
                    </div>
                </div>
            </div>
        </div>
